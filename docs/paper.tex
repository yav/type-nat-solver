...
information about the value being examined.
...
of SMT solvers form a user's perspective (Section~\ref{smt}); this
section may be skipped by readers familiar with the tools.  Then,
in Section~\ref{GHC}, we introduce the basic concepts of GHC's constraints
...
In this paper we describing a fairly general extension to the constraint
solver, however one could always imagine others too. Indeed, there is
existing work that extends GHC with support for units of measure
\cite{units-of-measure}.  Instead of having many ad-hoc extensions directly
...
that GHC has determine to be inert. This means that GHC simplified everything
as much as it could, improved using equalities, and basically cannot see
...
The constraints of interest to the algorithm are summarized by the following
grammar:
\begin{align*}
...
We are interested in working with natural number, however, the SMT
...
mustBeK s x v =
  solverPorve s (eq (SMT.const x) (value v))
\end{Verbatim}
...
The process of solving constraints is a straight-forward call the
...
with two call to the constraint solver: the first one asserts
...
GHC's constraint solver is not structured exactly in the same way as outlined
in the Nelson-Oppen simplification procedure, there are many similarities.
In particular, equalities play a central role in disseminating information,
but GHC currently does not do any back-tracking search (i.e., decision
procedure may only report inconsistency, or certain knowledge).  However,
there are situations where backtracking search is potentially useful.
For example, when solving class constraints in the context of overlapping
instances, or instance chains \cite{instance-chains}, it is useful
to consider the context of instances, but back-track and try a different
instance, if the context is found to be unsatisfiable.
...
as one could reason about the correctness of each decision separately,
but also, because it would make it simpler to experiment with new
